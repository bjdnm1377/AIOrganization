...
@pytest.mark.skipif(
    os.environ.get("AI_ORG_ENABLE_REAL_CODEX_STEPWISE_MULTI_FILE_TASK", "").lower() != "true",
    reason="real Codex CLI stepwise multi-file task test requires explicit opt-in",
)
@pytest.mark.skipif(shutil.which("codex") is None, reason="Codex CLI is not installed")
@pytest.mark.skipif(not _docker_available(), reason="Docker is not available")
def test_real_codex_stepwise_multi_file_task_creates_merge_candidate() -> None:
    ...
    project = service.create_project(
        CreateProjectRequest(
            title="Real Codex stepwise multi-file merge candidate task",
            goal="Verify controlled Codex stepwise multi-file orchestration",
            tasks=[
                TaskSpec(
                    title="Stepwise MergeCandidate summary path",
                    objective=(
                        "Run the controlled stepwise MergeCandidate task. "
                        "Each Codex step must edit only its allowed file. "
                        "Do not implement MergeService, merge, commit, push, "
                        "write docs, change config, or change dependencies."
                    ),
                    ...
                    acceptance_criteria=[
                        "real codex stepwise multi-file task reviewed independently"
                    ],
                )
            ],
        )
    ...
    assert metadata["codex_mode"] == "local_stepwise_multi_file_task"
    ...
    steps = metadata["stepwise_steps"]
    assert isinstance(steps, list)
    assert [step["allowed_files"] for step in steps] == [
        ["src/ai_org/adapters/codex/merge_candidate.py"],
        ["tests/unit/test_codex_merge_candidate.py"],
    ]
    assert all(step["main_worktree_fingerprint_consistent"] is True for step in steps)
